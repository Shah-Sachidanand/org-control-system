...
import { CreateMerchandiseRequest, UpdateMerchandiseRequest } from "../types";
...
router.get(
  "/",
  authenticate,
  checkFeatureAccess("merchandise", "read"),
...
router.post(
  "/",
  authenticate,
  checkFeatureAccess("merchandise", "write"),
  async (req: Request<{}, {}, CreateMerchandiseRequest>, res: Response) => {
    try {
...
router.put(
  ...
  checkFeatureAccess("merchandise", "write"),
  async (
    req: Request<{ id: string }, {}, UpdateMerchandiseRequest>,
    res: Response
  ) => {
    try {
      const currentUser = (req as any).user;
      ...
      if (
        currentUser.role !== "SUPERADMIN" &&
        merchandise.organizationId.toString() !==
          currentUser.organization._id.toString()
      ) {
        return res.status(403).json({ error: "Access denied" });
      }
...
router.delete(
  "/:id",
  authenticate,
  checkFeatureAccess("merchandise", "delete"),
  async (req: any, res: Response) => {
    try {
      const currentUser = req.user;
    ...
    try {
      ...
      switch (operation) {
        case "add":
          ...
          break;
        case "subtract":
          merchandise.inventory.quantity = Math.max(
            0,
            merchandise.inventory.quantity - quantity
          );
          break;
