    ...
    try {
      ...
      if (
        ...
        (!currentUser.organization ||
          currentUser.organization._id.toString() !== orgId)
      ) {
...
router.post(
  ...
  async (req: Request<{}, {}, CreatePartnerRequest>, res: Response) => {
    try {
...
router.put(
  ...
  authorize("ORGADMIN", "ADMIN", "SUPERADMIN"),
  async (
    req: Request<{ id: string }, {}, UpdatePartnerRequest>,
    res: Response
  ) => {
    try {
      const currentUser = (req as any).user;
...
router.delete(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (
        currentUser.role !== "SUPERADMIN" &&
        partner.organizationId.toString() !==
